...
if (!Module._mload_hook) {
	...
	Module._resolveLookupPaths = function(request, parent) {
		...
		if (resolveVersion && request && request != pkgName
			...
			if (aliasPath && aliasPath != request) {
				...
				return resolveVersion == 2 ? pathsMark : returnMark;
			}
		}

		var ret = _resolveLookupPaths.apply(Module, arguments);
		if (!resolveVersion && ret && ret[1]) {
			resolveVersion = typeof ret[1] == 'string' ? 2 : 1;
		}
		return ret;
	};
