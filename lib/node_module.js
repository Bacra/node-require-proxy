...
module.exports = function()
{
	if (!Module._mload_alias)
	{
		...
		{
			if (_asRequire && _asRequire != sPkgName && _aoParent && _aoParent.id && _aoParent.filename)
			{
				...
				for(var _sMloadPath in _oModuleAlias)
				{
					...
					if (_nIndex === 0 && _aoParent.filename.substring(_sMloadPath.length).indexOf('node_modules') == -1)
					{
						_oSubPath.push(_sMloadPath);
					}
					else
					{
						// console.log('node subpath', _oSubPath);
					}
				}
				...
				if (_oSubPath.length)
				{
					...
					// console.log('subpath', _oSubPath, getExistAlias(_asRequire, _oSubPath[0]));

					if (getExistAlias(_asRequire, _oSubPath[0]))
					{
						var _sAliasPath = _oModuleAlias[_asRequire];
						return ['mload_alias', ['mload_alias', _oSubPath[0]]];
					}
				}
				else
				{
					// console.log('no subpath', _asRequire, _oModuleAlias);
				}
			}
